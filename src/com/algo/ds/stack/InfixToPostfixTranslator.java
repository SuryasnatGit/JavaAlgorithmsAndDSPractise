 ...
 * 
 * Operator ')' - 
 * 					  If stack is not empty, then repeat the following:
 * 							Pop item from stack.
 * 							If item is not '(', then print to output.
 * 							If item is '(' then exit. 
 * 
 * Operator(this) - If stack is empty, then push to stack
 * 				  		  If stack is not empty, then repeat the following:
 * 								Pop item(top) from stack.
 * 								If item is '(' then push it again into stack.
 * 								Else, if top< this, then push top into stack
 * 								If top >= this, then print top to output.
 * 						 Quit loop if top < this or item is '('
 * 						  Push this.
 * No more items - 
 * 						  Pop the item and print to output.
 * 
...
public class InfixToPostfixTranslator {
	...
	public InfixToPostfixTranslator(int n) {
		maxSize = n;
		stackAsArray = new StackAsArray(maxSize);
	}
	...
	/**
	 * 
	 * @param c each character from the input string 
	 * @param i precedence number
	 */
