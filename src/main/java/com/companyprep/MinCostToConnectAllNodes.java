 ...
 * and the cost of addition, respectively (e.g. [1, 2, 5] means to add an edge between node 1 and 2, the cost would be
 * 5).
 * 
 * Example:
 * 
 ...
 * Explanation:
 * 
 * There are 3 connected components [1, 4, 5], [2, 3] and [6]. We can connect these components into a single component
...
public class MinCostToConnectAllNodes {
	...
	public static int minCost(int n, int[][] edges, int[][] newEdges) {
		UF uf = new MinCostToConnectAllNodes().new UF(n + 1); // + 1 because nodes are 1-based
		for (int[] edge : edges) {
