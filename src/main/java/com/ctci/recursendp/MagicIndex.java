...
public class MagicIndex {
	 ...
	 * @return
	 */
	public int magicIndex_bruteForce(int[] arr) {
	...
	/**
	 * if elements are not distinct then the above algo will fail. When we see that A[mid] < mid, we
	 * cannot conclude which side the magic index is on. It could be on the right side, as before. Or,
	 * it could be on the left side. The general pattern is that we compare midIndex and midValue for
	 * equality first. Then, if they are not equal, we recursively search the left and right sides as
	 * follows: Left side: search indices start through Math.min(midIndex-1,midValue).• Right side:
	 * search indices Math.max(midIndex+1,midValue) through end.
	 * 
	 * @param arr
	 * @return
	 */
