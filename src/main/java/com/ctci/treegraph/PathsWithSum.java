...
public class PathsWithSum {
	...
	/**
	 * In the brute force approach, we just look at all possible paths. To do this, we traverse to each
	 * node. At each node, we recursively try all paths downwards, tracking the sum as we go. As soon as
	 * we hit our target sum, we increment the total.
	 * 
	...
	/**
	 * 1. Track its runningSum. We'll take this in as a parameter and immediately increment it by node.
	 * value.
	 * 
	 * 2. Look up runningSum - targetSum in the hash table. The value there indicates the total number.
	 * Set totalPaths to this value.
	 * 
	 * 3. If runningSum == targetSum, then there's one additional path that starts at the root.
	 ...
	 * 
	 * 6. After we're done recursing left and right, decrement the value of runningSum in the hash
	 * table. This is essentially backing out of our work; it reverses the changes to the hash table so
	 * that other nodes don't use it (since we're now done with node).
	 * 
	 * The runtime for this algorithm is 0 (N), where N is the number of nodes in the tree. We know it
	 * is 0 (N) because we travel to each node just once, doing 0 (1) work each time. In a balanced
	 * tree, the space complexity is 0 (log N) due to the hash table. The space complexity can grow to
