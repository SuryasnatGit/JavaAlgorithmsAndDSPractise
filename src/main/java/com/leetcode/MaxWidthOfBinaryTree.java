...
public class MaxWidthOfBinaryTree {
	 ...
	 * @return
	 */
	int maxWidth(final TreeNode node) {
		return dfs(node, 0, 1, new ArrayList<>(), new ArrayList<>());
	}

	private int dfs(final TreeNode root, final int level, final int order, final List<Integer> start,
	 ...
	 * @return
	 */
	int maxWidthUsingHeapIndexing(TreeNode node) {
	...
	private void dfs(TreeNode node, int id, int depth, List<Integer> leftNodes, int maxWidth) {
		if (node == null)
			return;
	 ...
	 * @return
	 */
	int maxWidthUsingNodePositionOfBinaryTree(TreeNode node) {
		if (node == null)
			return 0;
		findMax(node, 0, 0);
		int res = 1;
	...
	private void findMax(TreeNode node, int level, int pos) {
		if (node == null)
			return;
	...
	int maxWidthOfBinaryTreeUsingBFS(TreeNode node) {
		if (node == null)
			return 0;
		int maxWidth = 0;
		// declare queue
		...
		while (!queue.isEmpty()) {
			...
			for (int i = 0, n = queue.size(); i < n; i++) {
				TreeNode key = queue.peek().getKey();
