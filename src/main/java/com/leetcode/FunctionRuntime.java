...
/**
 * Coding：给一些函数运行的log，计算每个函数的inclusive time（函数总运行时间）和exclusive time（函数总运行时间-子函数运行时间）。 例如
 * 
 * fun_name enter/exit time <br/>
 * f1 enter 1 <br/>
 * f2 enter 2 <br/>
 * f2 exit 3 <br/>
 * f1 exit 4 <br/>
 ...
 * Output:[3, 4]
 * 
 * Explanation: Function 0 starts at time 0, then it executes 2 units of time and reaches the end of time 1. Now
 * function 0 calls function 1, function 1 starts at time 2, executes 4 units of time and end at time 5. Function 0 is
 * running again at time 6, and also end at the time 6, thus executes 1 unit of time. So function 0 totally execute 2 +
 * 1 = 3 units of time, and function 1 totally execute 4 units of time.
 * 
 * Note: Input logs will be sorted by timestamp, NOT log id. Your output should be sorted by function id, which means
 * the 0th element of your output corresponds to the exclusive time of function 0. Two functions won't start or end at
 * the same time. Functions could be called recursively, and will always end. 1 <= n <= 100
 * 
 *
 */
