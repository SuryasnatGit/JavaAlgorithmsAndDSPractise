...
/**
 * 
 * Given a 2 D floor plan with empty space, block and multiple exits. Find distance of every empty
 * space from nearest exits in case of fire emergency.
 * 
 * Idea is to start from every exit position and do BFS search in all 4 directions and maintain the
 * distance of every space from exit. If another exit in future iterator is closer than already
 * calculated exit then update the distance.
 * 
 * Space complexity is O(n*m) Time complexity is O(number of exit * m * n);
 *
 */
