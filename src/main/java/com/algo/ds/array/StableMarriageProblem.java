 ...
 * sex who would both rather have each other than their current partners. If there are no such people, all the marriages
 * are stable.
 * 
 * It is always possible to form stable marriages from lists of preferences (See references for proof). Following is
 * GaleShapley algorithm to find a stable matching: The idea is to iterate through all free men while there is any free
 * man available. Every free man goes to all women in his preference list according to the order. For every woman he
 * goes to, he checks if the woman is free, if yes, they both become engaged. If the woman is not free, then the woman
 * chooses either says no to him or dumps her current engagement according to her preference list. So an engagement done
 * once can be broken if a woman gets better option.
 * 
 ...
 * 
 * In the first round, first each unengaged man proposes to the woman he prefers most, and then each woman replies
 * "maybe" to her suitor she most prefers and "no" to all other suitors. She is then provisionally "engaged" to the
 * suitor she most prefers so far, and that suitor is likewise provisionally engaged to her.
 * 
 * In each subsequent round, first a) each unengaged man proposes to the most-preferred woman to whom he has not yet
 * proposed (regardless of whether the woman is already engaged), and then b) each woman replies "maybe" if she is
 * currently not engaged or if she prefers this man over her current provisional partner (in this case, she rejects her
 * current provisional partner who becomes unengaged). The provisional nature of engagements preserves the right of an
