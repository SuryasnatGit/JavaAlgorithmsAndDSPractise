...
/**
 * 
 * Given an array representation of binary tree where index is data while value at index is parent
 * create the binary tree. Value of -1 indicates root node.
 * 
 * References:
 * http://www.geeksforgeeks.org/construct-a-binary-tree-from-parent-array-representation/.
 * 
 * Given an array that represents a tree in such a way that array indexes are values in tree nodes and array values give the parent 
 * node of that particular index (or node). The value of the root node index would always be -1 as there is no parent for root. 
