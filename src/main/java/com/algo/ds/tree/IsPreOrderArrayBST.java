 ...
 *
 * Idea is to keep minimal acceptable value. Any element in pre array should never be less than
 * minimal acceptable value. Values are popped from stack till they are less than pre[i] Last popped
 * value becomes new minimally acceptable value.
 * 
 ...
 *
 * Time complexity - O(n)
 * https://leetcode.com/problems/verify-preorder-sequence-in-binary-search-tree/ Reference -
...
public class IsPreOrderArrayBST {
	...
	public boolean verifyPreorder(int pre[]) {
		...
		for (int i = 1; i < pre.length; i++) {
			// if we find a node which is on right side and smaller than minAcceptible return false
