...
public class LowestCommonAncestorInNaryTree {
	 ...
	 * 
	 * 第一题问最低公共节点。LC236。 然后是问最深节点的最低公共节点。不一定是Binary Tree，做出来了面试官没说有什么问题然后时间到了让分析一下复杂度。 跟二叉树一样，不用先求深度，可以one pass。
	...
	/*
	 * Iterative 学习一下迭代的方式 use a hashmap to record parent of every node. do BFS and record the most left node and most
	 * right node of each level Then we have the most left and right node of the last Level search into the hashMap
	 * until we find the same parent which is the LCA
	 */
	public static TreeNode M1_LCAAllDeepestLeaves_Iter(TreeNode root) {
		...
		if (leftMost == rithtMost) { // How could they be the same? deepest level has only 1 node
			...
			while (leftP != null && rightP != null && leftP != rightP) { // backtrack, Nice! No need to check null here
																			// since worst case is root
				leftP = map.get(leftP);
