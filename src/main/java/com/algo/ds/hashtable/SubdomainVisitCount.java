...
public class SubdomainVisitCount {
	...
	public Map<String, Integer> subDomainCount(List<String> domains) {

		Map<String, Integer> domainMap = new HashMap<>();
		for (String domain : domains) {
			int domainCount = Integer.valueOf(domain.substring(0, domain.indexOf(" ")));
			String domainStr = domain.substring(domain.indexOf(" "));
			System.out.println("domainStr :" + domainStr);
			String[] split = domainStr.split("\\.");
			int size = split.length;
			String subDomain = "";
			for (int i = size - 1; i >= 0; i--) {
				subDomain = split[i] + (i < size - 1 ? "." : "") + subDomain; // tricky
				domainMap.put(subDomain, domainMap.getOrDefault(subDomain, 0) + domainCount);
			}
	...
	public static void main(String[] args) {
		...
		System.out.println(sd.subDomainCount(domains));
	}
}
