...
/**
 * Multiply two numbers in form of link list. Idea is to multiply one number from head2 with all
 * numbers from head1. This result is stored in currentResult. Pass this currentResult and any
 * previous result from multiplication and add them.
 * 
 * Examples:
 * 
 * Input : 9->4->6 8->4 Output : 79464
 * 
 * Input : 3->2->1 1->2 Output : 3852
 * 
 */
public class MultiplyTwoNumbersLinkList {
	...
	public Node multiply(Node head1, Node head2) {
		...
		while (head2 != null) { // for each node of 2nd list
			Node current = head1;
			int carry = 0;
			while (current != null) { // for each node of 1st list
				int r = current.data * head2.data;
				r += carry;
				carry = r / 10;
				r = r % 10;
				if (currentResult == null) {
					currentResult = dll.addNodeAtFront(currentResult, r);
			...
			if (carry != 0) {
				currentResult = dll.addNodeAtFront(currentResult, carry);
			}
			currentResult = null;
			if (result == null) {
