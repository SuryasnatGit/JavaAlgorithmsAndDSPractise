...
public class SkipList<E> {
	...
	/**
	 * Create a random number function from the standard Java Random class. Turn it into a uniformly distributed value
	 * within the range 0 to n-1 by taking the value mod n.
	 * 
	 * @param n
		...
		public void printForward() {
			for (int i = this.forward.length - 1; i >= 0; i--) {
				if (this.forward[i] == null)
					...
					System.out.println("level : " + i + " ----> (" + this.forward[i].key() + " : "
							+ this.forward[i].element() + ")");
			}
		}
