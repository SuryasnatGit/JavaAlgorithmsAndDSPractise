...
public class InsertIntoCycledLinkedList {
	...
	ListNode Solution(ListNode head, int toInsert) {
		...
		while (cur.next != head) {
			...
			if (toInsert >= cur.data && toInsert <= next.data) {
				cur.next = newNode;
				newNode.next = next;
			...
			if (cur.data > next.data) { // Find the gap
				if (toInsert >= cur.data || toInsert <= next.data) { // Largest or
																		// smallest
					cur.next = newNode;
					newNode.next = next;
	...
	public ListNode learnFromOthers(ListNode head, int val) {
		...
		do {
			...
			if (val >= cur.data && val <= cur.next.data)
				break;
			// 如果正好是排序链断开处
			if (cur.data > cur.next.data && (val < cur.next.data || val > cur.data))
				break;
			// 否则继续往下找
			cur = cur.next;
		} while (cur != head);
	...
	/*
	 * 给出 ［“1999-02-13”， “2001-03-14"］ 打印出中间所有的月份 ［“1999-02-13”， “1999-02-28"]，［“1999-03-01”，
