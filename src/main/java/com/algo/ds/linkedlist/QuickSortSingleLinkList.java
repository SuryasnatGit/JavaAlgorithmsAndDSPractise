 ...
 * 
 * Test cases 0 1 or more nodes sorted reverse sorted nodes
 */

// keep head and tail of each result since caller function needs it to
// set complete linked list. If we do not keep tail in each recursion we will
...
public class QuickSortSingleLinkList {
	...
	public Node quickSortSlower(Node head) {
		...
		smaller = quickSortSlower(smaller);
		larger = quickSortSlower(larger);

		Node tail1 = smaller;

		// this is costly operation which can be prevented by keeping tail in each recursion
