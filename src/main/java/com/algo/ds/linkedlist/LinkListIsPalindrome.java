...
public class LinkListIsPalindrome {
	...
	/**
	 * Time Complexity: O(n) Auxiliary Space: O(n) if Function Call Stack size is considered, otherwise
	 * O(1).
	 * 
	 * Use two pointers left and right. Move right and left using recursion and check for following in
	 * each recursive call. 1) Sub-list is palindrome. 2) Value at current left and right are matching.
	 * 
	 ...
	 * node from head. We advance the head pointer in previous call, to refer to next node in the list.
	 * 
	 ...
	 * head pointer for reflecting the changes in parent recursive calls.
	 * 
	 * @param head
	 * @param end
	 * @return
