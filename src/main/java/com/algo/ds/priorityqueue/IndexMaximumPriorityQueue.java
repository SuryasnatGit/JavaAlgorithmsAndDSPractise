...
public class IndexMaximumPriorityQueue<Key extends Comparable<Key>> implements Iterable<Integer> {
	 ...
	 *             if {@code maxN < 0}
	 */
	public IndexMaximumPriorityQueue(int maxN) {
		if (maxN < 0)
	...
	private class HeapIterator implements Iterator<Integer> {
		// create a new pq
		private IndexMaximumPriorityQueue<Key> copy;

		// add all elements to copy of heap
		// takes linear time since already in heap order so no keys move
		public HeapIterator() {
			copy = new IndexMaximumPriorityQueue<Key>(pq.length - 1);
			for (int i = 1; i <= n; i++)
