...
public class IndexMinimumPriorityQueue<Key extends Comparable<Key>> implements Iterable<Integer> {
	 ...
	 *             if {@code maxN < 0}
	 */
	public IndexMinimumPriorityQueue(int maxN) {
		if (maxN < 0)
	...
	private class HeapIterator implements Iterator<Integer> {
		// create a new pq
		private IndexMinimumPriorityQueue<Key> copy;

		// add all elements to copy of heap
		// takes linear time since already in heap order so no keys move
		public HeapIterator() {
			copy = new IndexMinimumPriorityQueue<Key>(pq.length - 1);
			for (int i = 1; i <= n; i++)
